...
***************************************************************************
  last update 20201206 by ericBcreator
***************************************************************************
*/
...
void checkAlarms() {
    ...
    if (firstAlarmSet || forceFirstAlarm) {     // 1st alarm activated or forced      
      ...
      switch (alarmList[alarmNum].actionType) {     
        case actionGPIOhigh:                        // set GPIO pin high
          ...
          //custom action marcel
          //publish to mqtt
          //  Serial.println("Now we publish to MQTT");
          ...
          break;
        case actionGPIOlow:                         // set GPIO pin low Das hier werde ich machen marcel marcel marcel marcel.Setzte action pin low, da input pullup
  ...
  if (alarmTriggered) {// loop code
    digitalWrite(alarmPin,LOW); //marcel alarmpin allgemein
    if (alarmList[alarmNum].duration == 666) {
      displayTime();
      displayAnimPacMan();
    ...
    if (alarmTriggered) {                                                                           // check if alarm is still triggered
      if (alarmList[alarmNum].actionType == 0 || alarmList[alarmNum].actionType == actionNeoOn) {   // don't play the buzzer in the loop for actions marcel hier könnte man machen, dass der alarm währen aktionen weiter läuft
        if (alarmList[alarmNum].buzzer > 0 && !alarmMessage) {                                      // play the buzzer every other sec when it is set
          if ((millis() - prevBuzzerTime) >= 2000) {
            if (alarmmute==false){// and there is no message set
    ...
    if ((millis() - alarmTriggerTime) >= (alarmList[alarmNum].duration * 1000)) {    //marcel der alarm ended, wenn die zeit verstichen ist
      DEBUGPRINTLN("Alarm " + fillSpace(alarmNum, 2)+ "   ended   at " + timeString); // vielleicht kann man auch machen, dass er endet, wenn mqtt funkt. marcel marcel marcel. ich glaub das ist es
      alarmEnded(); // marcel einfach noch ei if statement hierhin
      
    }
//mein pushbutton soll den alarm ausschalten
...
void unmutealarm(){
  if (alarmmute==true&&(millis() > lastmute + mutetime)){
...
void alarmEnded() {
  ...
  if (alarmList[alarmNum].duration > 0 || alarmIsInterrupted) {    // if duration is > 0 or the alarm is interrupted, reset gpio or neopixels otherwise keep it in the current state
    digitalWrite(alarmPin,HIGH); //marcel alarmpin off allgemein
