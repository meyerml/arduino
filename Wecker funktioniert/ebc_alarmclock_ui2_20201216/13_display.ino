...
void displayAlarmTime(String message) {  
  ...
  //    d2d_City, msgSunrise, d2dSunrise, msgSunset, d2dSunset, msgDaylightTime, d2dDaylightTime, msgDaylightTimeDifference, daylightTimeDiff);
    ...
    if (displayDaylightInfo) {
      ...
      daylightInfoMessage.replace("<br>", "");  // remove the the line for the display
...
//marcel hier kann der read teil hin, statt dem rotary switch, wird dann die mqtt subscription abgefragt.
      #ifndef NO_ENCODER
...
void displayAnimPacMan(bool reverse) {
  ...
  int state = 0, column, maxColumn, charWidth;  
