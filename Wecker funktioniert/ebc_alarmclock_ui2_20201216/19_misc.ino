...
***************************************************************************
  last update 20201212 by ericBcreator
***************************************************************************
*/
...
void delayAndHandle(int delayTime) {
  ...
  while (millis() - startDelayTime < delayTime) {
    if (millis() - startDelayTime > handleEventsDelay) { 
...
//marcel eventuell einfach sagen, wenn mqtt, dann tue so, als wäre der rotary encoder gedrückt. oder einen neuen eintrag hier, der heist:
// wenn mqtt ankommt, calle alarminterrupted()
int handleEvents() {
  #ifdef ENABLE_PIR
...
//hier könnte man auch das mqtt signal einfügen... NEIN:Das hier sagt nur im allgemeinen, was paassieren soll, 
//wenn der rot switch gepresst wird. eventuell entfernen, so dass man dden wecker nicht mehr mot dem rotary encoder ausmachen kann.
...
//          UI_rereadTab(); // @EB-todo reread
      ...
      if (reSwitch || reEnc ) {// marcel pushbutton im rotenc-pelz
        ...
        if (sleeping) {
          ...
          if (alarmTriggered) {
            ...
            //alarmInterrupted(); mute the alarm
            mutealarm();
          }  
          else {
            if (reSwitch == 1) {
      ...
      //additional if for pushbutton marcel
...
//  while ((millis() - startDelayTime) < duration) { };
  delayAndCheckEnc(duration); //marcel hier wird geguckt, ob der encoder gedrückt wird. evtl hier mqtt checken. hmm aber wir sind hier in void playtone...
