...
*******************************************************************************
  Main file - includes, setup and loop - last update 20201216 by ericBcreator
*******************************************************************************
  My setup:
  - Wemos D1 mini V3 / ESP32 mini
...
//#include "05_language_dut.h"              // @EB-setup
#include "05_language_eng.h"              // @EB-setup language
...
void setup() {
  ...
  DEBUGPRINTLN((String) LED_NUM_OF_VERT_DISPLAYS + " vertical led matrices ");
  setFeatures += "-" + String(matrix.width()) + " columns of leds";
  ...
  constrain(ls_minValue, 0, 15);
  constrain(ls_maxValue, 0, 15);  
...
void loop() {
  ...
  if (alarmMessage) {
    ...
    while (((millis() - displayInfoStartTime) / 1000 < displayInfoInterval)) {
      ...
      delayAndCheckEnc(mainLoopDelay);          // @EB-todo || marcel hier wird immer abgefragt, was der rotary encoder macht. hier könnte man was allgemeines einfügen JAP, klappt
      if(endalarm == true){
        endalarm=!endalarm;
        alarmInterrupted();
         sleepMode = false;
        //hierhin
           ...
           /* digitalWrite(ONBOARD_LED,HIGH);
            delay(1000);
            digitalWrite(ONBOARD_LED,LOW);      // das wird aber immer abgefragt. besser wäre eine stelle, wo nur kommt, wenn der alarm läuft 
            delay(1000);*/
