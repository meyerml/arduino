...
/*
  ...
  * stops appearing (only the "Control connected" message is visible, the rest of the page is blank)
  the problem is issued at the ESPUI github page, but there is no solution yet (as of 22 Oct 2020)
  defining UI_CONTROLS_MINIMAL will minimize the number of controls
...
//#define CLOCK_ESP32_MINI_TEST                 // ESP32 mini test. note: GPIO 12 single led
...
#else // default setup
//#define LED_NUM_OF_HOR_DISPLAYS 4             // @EB-setup defaults to 4, can be more, set to 8 for 2 4x8x8 matrices
...
//#define LIGHTSENSOR_MAX_VALUE 15              // @EB-setup brightness max value (0-15). will be set to 15 if not defined
  ...
  #define PIR_DEFAULT LOW                       // @EB-setup set to HIGH if the motion sensor defaults to HIGH or else LOW
